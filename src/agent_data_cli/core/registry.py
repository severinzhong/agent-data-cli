...
from agent_data_cli.core.capabilities import CapabilityResolver
from agent_data_cli.core.config import (
    ConfigCheckItem,
...
class SourceRegistry:
    def __init__(self, store: Store | None) -> None:
        ...
        self._failed_sources: dict[str, FailedRegisteredSource] = {}
    ...
    def list_descriptors(self) -> list[SourceDescriptor]:
        ...
        for name in sorted(self._failed_sources):
            failure = self._failed_sources[name]
            descriptors.append(
                SourceDescriptor(
                    name=failure.name,
                    display_name=failure.name,
                    summary=failure.error,
                    effective_mode="import_error",
                    action_statuses={
                        "channel.search": CapabilityStatus("unsupported", reason="import_error"),
                        "content.search": CapabilityStatus("unsupported", reason="import_error"),
                        "content.update": CapabilityStatus("unsupported", reason="import_error"),
                        "content.query": CapabilityStatus("unsupported", reason="import_error"),
                        "content.interact": CapabilityStatus("unsupported", reason="import_error"),
                    },
                )
            )
    ...
    def _require_source(self, name: str) -> RegisteredSource:
        failed = self._failed_sources.get(name)
        if failed is not None:
            raise RuntimeError(f"source failed to load: {name}: {failed.error}")
        try:
...
def build_default_registry(store: Store | None, sources_dir: Path | None = None) -> SourceRegistry:
    ...
    scan = scan_source_modules(resolved_sources_dir)
    for failure in scan.failures:
        registry._failed_sources[failure.name] = FailedRegisteredSource(
            name=failure.name,
            error=failure.error,
        )
