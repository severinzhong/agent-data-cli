...
- core 负责把 `--since` 解析为统一时间下界，再传给 query/source
...
- 如果多 source 查询时 `query.view_id` 一致，CLI 可以合并渲染
- 如果 `query.view_id` 不一致，CLI 退回通用时间线视图
...
- `search_channels(query: str, limit: int) -> list[ChannelRecord | SearchResult]`
- `search_content(channel_key: str | None, query: str | None, since: str | None, limit: int) -> list[SearchResult]`
- `fetch_content(channel_key: str, since: str | None, limit: int | None, fetch_all: bool) -> list[ContentRecord]`
...
- `SearchResult` 至少包含 `source/result_kind/title/url`，内容结果还要包含 `content_ref`
...
- `<percent-encoded-source-id>` 使用 URL percent-encoding 保存 source 自己的 opaque id
- core 只校验：
  - 前缀 source 是否存在且合法
  - 是否包含固定分隔 `:content/`
