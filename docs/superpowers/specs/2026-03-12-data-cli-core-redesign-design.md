...
这次重构除了协议边界，还要显式约束编码风格。目标是做出一个小而硬的 core，后续开源时可以合理强调“用较少代码完成完整数据中心 CLI 核心”，但不为了压缩行数牺牲能力和边界。

约束：

- 优先减少“概念数量”，不是只减少“语法行数”
- 新增抽象前先问：是否真的消除了重复，且至少服务两个 source 或两个 core 场景
- 优先普通 dataclass / 函数 / 显式流程，不引入元类、注册魔法、复杂 decorator 协议
- parser / help / config check / capability check 必须共享同一份 manifest 真相，不允许各写一套
- 不写兼容层、fallback 层、旧命令适配层
- 不做“wrapper 套 wrapper”的抽象堆叠
- core 模块要保持单一职责，能直接读懂执行路径
- 行数是次级指标，清晰边界和完整能力是一级指标

实现期的判断标准：

- 如果一个能力可以通过删掉一层抽象获得更少代码和更清晰逻辑，优先删抽象
- 如果为了少几行代码引入隐式行为、动态魔法或难测逻辑，则不接受
...
- `--group` 只作用于本地库过滤，不触发远端展开或订阅检查
- `query --group` 的 group 语义与 update 不同：
  - `source` 成员表示该 source 的全部本地记录
  - `channel` 成员表示该单个 channel 的本地记录
  - 二者合并后按 `(source, channel_key)` 去重
...
- `required_record_fields` 至少包含 `source/channel_key/title/url/fetched_at/raw_payload/dedup_key`
- 如果 source 声明了 `content.interact`，则 `required_record_fields` 还必须包含 `content_ref`
- `published_at` 是否必需，取决于 `query.time_field` 是否为非空
- `unique_key_fields` 固定默认值为 `(source, dedup_key)`，作为 upsert 与去重键
...
- manifest 中的 option / param `name` 都使用 canonical internal id，必须匹配 `^[a-z][a-z0-9_]*$`
- CLI flag 通过 canonical id 自动映射为 kebab-case，例如 `dry_run -> --dry-run`
- 私有 param 名不能占用保留 internal id：`source/group/channel/query/keywords/since/limit/all/jsonl/dry_run/verb/ref/help/mode/name`
...
- `type=bool` 时，`multiple` 必须为 `False`
- `type=bool` 时，`required` 必须为 `False`
...
- `SearchResult` 只用于 `content search`，至少包含 `source/result_kind/title/url`
- 如果 source 声明了 `content.interact`，则 `SearchResult` 必须额外包含 `content_ref`
- `ContentRecord` 至少包含 `source/channel_key/title/url/fetched_at/raw_payload/dedup_key`
- 如果 source 声明了 `content.interact`，则 `ContentRecord` 必须额外包含 `content_ref`
- 如果 source 的 `query.time_field` 为非空，则 `ContentRecord` 必须提供对应时间字段，例如 `published_at`
...
- 只有声明了 `content.interact` 的 source 才必须实现 `parse_content_ref()`
...
只有声明了 `content.interact` 的 source，才强制要求提供一等稳定可执行标识 `content_ref`。不支持 interact 的 source 可以完全不提供 `content_ref`。

当 source 需要提供 `content_ref` 时，格式为：
...
如果 source 提供了 `content_ref`，它既可来自本地库记录，也可来自远端 search 结果；core 不要求它必须先落库。

### 记录字段调整

只有声明了 `content.interact` 的 source 才要求本地 content 记录持久化 `content_ref`。

`external_id` / `dedup_key` 仍可保留给 source 内部和 store 去重使用；只有在 source 支持 interact 时，core 才对外以 `content_ref` 作为可执行引用。
